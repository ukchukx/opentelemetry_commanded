defmodule OpentelemetryCommanded.EventHandlerTest do
  ...
  describe "dispatch command when Telemetry attached" do
    setup _ do
      case OpentelemetryCommanded.EventHandler.setup() do
        :ok -> :ok
        ...
        capture_log(fn ->
          :ok =
            app_dispatch(context, %C.DoEvent{
              id: "ACC123",
              event: %E.ErrorInEventHandlerEvent{message: "some error"}
            })
        end)
      ...
      _log =
        capture_log(fn ->
          :ok =
            app_dispatch(context, %C.DoEvent{
              id: "ACC123",
              event: %E.ExceptionInEventHandlerEvent{message: "some error"}
            })
        end)
